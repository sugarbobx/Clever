...
const pwdSchema = yup.object({
  ...
  new_password: yup.string().min(8, 'Min 8 characters.').matches(/[A-Z]/, '1 uppercase required.').matches(/[0-9]/, '1 number required.').required('New password is required.'),
});
...
export default function ClientProfile() {
  ...
  const { register: regP, handleSubmit: handleP, formState: { errors: errP, isSubmitting: subP }, reset: resetP } = useForm({
    resolver: yupResolver(profileSchema),
  });
  const { register: regW, handleSubmit: handleW, formState: { errors: errW, isSubmitting: subW }, reset: resetW } = useForm({
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-10">
      ...
      <h1 className="text-2xl font-semibold text-[#1A3C34] tracking-tight mb-8">My Profile</h1>
